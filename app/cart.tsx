    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      ...
      >
        ...
        <View style={styles.groupHead}>
          <Text style={styles.restName}>
            {restaurantName ?? restaurant.data?.name ?? "Your order"}
          </Text>
          {restaurant.data ? (
            <Text style={styles.meta}>
              {etaLabel(restaurant.data)} delivery
            </Text>
          ) : null}
        </View>
        ...
        <View style={styles.summary}>
          ...
          <Row
            label="Delivery fee"
            value={
              pricing.deliveryFeeMinor === 0
                ? "Free"
                : formatMinor(pricing.deliveryFeeMinor)
            }
            hint={
              pricing.freeDeliveryApplied ? "Free delivery unlocked" : undefined
            }
          />
          <Row
            label="Service fee"
            value={formatMinor(pricing.serviceFeeMinor)}
          />
      ...
      <View style={[styles.footer, { paddingBottom: theme.spacing.lg }]}>
        <Button
          ...
          trailing={formatMinor(pricing.totalMinor)}
          onPress={onCheckout}
        />
      </View>
    </SafeAreaView>
      ...
      <View style={styles.line}>
        {line.image ? (
          <Image
            source={line.image}
            placeholder={
              line.blurhash ? { blurhash: line.blurhash } : undefined
            }
            contentFit="cover"
            style={styles.thumb}
          />
          ...
          <View
            style={[styles.thumb, { backgroundColor: theme.colors.bgMuted }]}
          />
        )}
        <View style={styles.lineBody}>
          ...
          {line.optionsSummary ? (
            <Text style={styles.lineOpts} numberOfLines={1}>
              {line.optionsSummary}
            </Text>
          ) : null}
          {line.notes ? (
        ...
        <QuantityStepper
          ...
          allowRemove
          size="sm"
          accessibilityLabel={`${line.name} quantity`}
        />
      </View>
    </Swipeable>
  );
...
function Row({
  ...
  return (
    <View style={styles.row}>
      <View style={{ flex: 1 }}>
        <Text style={bold ? styles.rowLabelBold : styles.rowLabel}>
          {label}
        </Text>
        {hint ? (
          <Text style={[styles.rowHint, { color: theme.colors.success }]}>
            {hint}
          </Text>
        ) : null}
      </View>
