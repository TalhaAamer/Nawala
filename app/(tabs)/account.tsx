...
import {
  ...
  upsertAddress,
  removeAddress,
} from "@/services/storage/addresses";
...
export default function Account() {
  ...
  const name = isUser
    ? (user.displayName ?? user.email ?? user.phone ?? "Nawala member")
    : "Guest";
  ...
  useEffect(() => {
    ...
    getJSON<NotificationPrefs>(STORAGE_KEYS.notifications, DEFAULT_PREFS).then(
      setPrefs,
    );
  }, []);

  const favRestaurants = (restaurants.data ?? []).filter((r) =>
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.profileCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sub}>
              {isUser ? "Nawala member" : "Browsing as guest"}
            </Text>
          </View>
          {!isUser ? (
            <Button
            ...
            {cards.map((c) => (
              <View key={c.id} style={styles.listRow}>
                <Ionicons
                  ...
                  color={theme.colors.textSecondary}
                />
                <Text style={[styles.rowPrimary, { flex: 1 }]}>
                  {c.brand} {c.label}
                </Text>
              </View>
            ))}
            <Text style={styles.demoNote}>
              Demo build — payment is not real.
            </Text>
          </Section>
        ) : null}
        ...
        <Section title="Notifications">
          ...
          <ToggleRow
            label="Promos & deals"
            value={prefs.promos}
            onChange={(v) => updatePref("promos", v)}
          />
          <ToggleRow
            label="New restaurants near me"
            value={prefs.nearby}
            onChange={(v) => updatePref("nearby", v)}
          />
        </Section>
        ...
        <Section title="About">
          ...
          <Text style={styles.version}>
            Nawala v{Constants.expoConfig?.version ?? "1.0.0"} · demo build
          </Text>
        </Section>
        ...
        {__DEV__ ? (
          <Button
            label="Open kitchen sink (dev)"
            variant="secondary"
            onPress={() => router.push("/_kitchen-sink")}
          />
        ) : null}
      ...
      >
        <AddressForm
          ...
          onDelete={
            editingAddress
              ? () => onDeleteAddress(editingAddress.id)
              : undefined
          }
        />
      </Sheet>
      ...
      >
        ...
        />
        <Text style={styles.demoNote}>
          Demo build — only the last 4 digits are stored locally.
        </Text>
      </Sheet>
      ...
      >
        <Text style={styles.legalText}>
          This is a demo build of Nawala. No real account, payment, or delivery
          is provided. All data lives on your device and can be cleared at any
          time. {legalTitle} content is placeholder text for demonstration only.
        </Text>
...
function AddressForm({
  ...
  const save = () => {
    const finalAddress: Address = {
      ...form,
      id: form.id || `addr_${Date.now().toString(36)}`,
    };
  ...
  return (
    <View style={{ gap: theme.spacing.sm }}>
      ...
      <View style={{ flexDirection: "row", gap: theme.spacing.sm }}>
        ...
        <View style={{ flex: 1 }}>
          {field("zip", "ZIP", { keyboardType: "number-pad" })}
        </View>
      </View>
      {error ? <Text style={styles.formError}>{error}</Text> : null}
      <Button label="Save address" onPress={save} />
      {onDelete ? (
        <Button label="Delete address" variant="ghost" onPress={onDelete} />
      ) : null}
    </View>
  );
}
